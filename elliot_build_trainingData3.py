...
def main():
    ...
    #33 = TRB/G, 34 = AST/G, 35 = TOV/G, 36 = FLS/G
    basic_stats_matrix = ReadBasicStatsToMatrix('data/sports-reference/basic_stats.csv', [1, 5, 6, 18, 21, 24, 33, 34, 35, 36]) #
    ...
    (X,y) = (training_data[:,range(18)], training_data[:,18]) #
    print X
    print y
    ...
    predictions = MakePredictions(model, test_data[:,range(18)]) #
    accuracy = GetAccuracy(predictions, test_data)
    print "LogisticRegression Accuracy: ", accuracy

    predictions2 = MakePredictionsSVM(model2, test_data[:,range(18)])
...
def GetAccuracy(predictions, test_data):
    ...
    for index, prediction in enumerate(predictions):
        if(prediction == test_data[:,18][index]): #
...
def buildTrainingDataMatrix(stats_matrix, games_matrix):
    training_data_matrix = np.zeros(shape = (len(games_matrix), 19)) #
    for game_index, game in enumerate(games_matrix):
        ...
        for stats_index, team_id in enumerate(stats_matrix[:,0]):
            ...
            if(team_id == team2):
                training_data_matrix[game_index][9] = stats_matrix[stats_index][1] #
                training_data_matrix[game_index][10] = stats_matrix[stats_index][2] #
                training_data_matrix[game_index][11] = stats_matrix[stats_index][3] 
                training_data_matrix[game_index][12] = stats_matrix[stats_index][4]
                training_data_matrix[game_index][13] = stats_matrix[stats_index][5]
                training_data_matrix[game_index][14] = stats_matrix[stats_index][6] 
                training_data_matrix[game_index][15] = stats_matrix[stats_index][7] #
                training_data_matrix[game_index][16] = stats_matrix[stats_index][8] 
                training_data_matrix[game_index][17] = stats_matrix[stats_index][9] 
        training_data_matrix[game_index][18] = win_loss #
    ...
    for row_index, data_row in enumerate(training_data_matrix):
        if(data_row[0] == 0 or data_row[9] == 0): #
